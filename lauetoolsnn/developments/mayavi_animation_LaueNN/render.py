...
model_direc = r"C:\Users\purushot\Desktop\python_animation\mayavi_version\LaueNN_model\Cu"
...
max_neurons = np.max((codebars.shape[1], fc1.shape[1], fc2.shape[1], fc3.shape[1], output.shape[1]))
# Layer units
...
for i in tqdm(list(range(len(codebars)))):
    ...
    # =============================================================================
    spot_plot = i
    list_ = np.where((tab_angulardist[spot_plot,:] - 20) > 20/2.)[0]
    ...
    mlab.points3d(np.zeros_like(l_tth), l_tth, l_chi, color=(0,0,0),
    ...
    connections_img = mlab.pipeline.surface(lines_img, color=(1,0,0), line_width=1, opacity=1)
